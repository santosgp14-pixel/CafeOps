...
function CalibTab({ logs, setLogs, auditLogs, setAuditLogs, showToast }) {
  ...
  const ratio = form.inputG && form.outputG
    ...
    <div className="main">
      ...
      {showForm && (
        <div className="inline-form">
          <div className="inline-form-title">⚖️ Nuevo registro de calibración</div>
            ...
            {ratio && (
              <>
                ...
                <div className="calib-chip" style={{ background: 'var(--espresso)', border: 'none' }}>
                  ...
                  <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--latte)' }}>1:{ratio}</div>
                </div>
              </>
            )}
          </div>
            ...
            return (
              <div key={entry.id} className="card">
                <div className="card-row">
                  ...
                  <button className="btn btn-danger btn-icon btn-sm" onClick={() => requestDelete(entry)}>🗑</button>
                </div>
              </div>
            );
          })}
